...
(* CHANGE: Add a case for Shift expressions *)
	  | Shift(dx, dy, e) => case (eval_prog(e, env)) of
							  ...
							  | _ => raise BadProgram("Shift applied non-values")
...
fun preprocess_prog (e) =
	case e of
		LineSegment seg =>  
			let 
				val (xStart, yStart, xEnd, yEnd) = seg
			in 
				if real_close_point (xStart, yStart) (xEnd, yEnd)
				then Point(xStart, yStart)
				else if (xStart > xEnd)
				then LineSegment(xEnd, yEnd, xStart, yStart)
				else if real_close (xStart, xEnd) andalso (yStart > yEnd)
				then LineSegment(xStart, yEnd, xEnd, yStart)
				else e
			end
